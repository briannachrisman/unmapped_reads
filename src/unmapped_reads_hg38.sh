...
if [ ! -f ${unmapped_path}/$HOST/$HOST.bam.done ]; then
    if [ "$(aws s3 ls ${aws_path}/$HOST.final.cram | wc -l)" -ge "1" ]; then
        ...
        \rm ${unmapped_path}/$HOST/$HOST.bam
        samtools view s3://${aws_path}/$HOST.final.cram -f 4 -b -o ${unmapped_path}/$HOST/$HOST.bam
    fi
fi
...
out_path=${unmapped_path}/${HOST}/${HOST}
